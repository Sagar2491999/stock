...
import javax.swing.*;
import java.sql.*;
class StockModify extends Frame implements ActionListener
{
JLabel jl=new JLabel("Modify Items! ");
...
Font fb=new Font("",2,23);
Connection con;
PreparedStatement st;
ResultSet rs;
String icode,iname,idesc,dt;
int quty,rol;
float cp,sp,pft;
StockModify()
{
...
b1.setBounds(50,930,110,40);add(b1);
b1.setFont(fb);add(b1);
b2.setBounds(200,930,110,40);add(b2);
b2.setFont(fb);add(b2);
b3.setBounds(350,930,110,40);add(b3);
...
rol=Integer.parseInt(t5.getText());
cp=Float.parseFloat(t6.getText());
pft=Float.parseFloat(t7.getText());

sp=cp+((cp*pft)/100);

//sp=Integer.parseInt(t8.getText());
dt=t9.getText();
try
{
st=con.prepareStatement("update  iteminfo set itemname=?,itemdescription=?,quantity=?,reorderlevel=?,costprice=?,profit=?,saleprice=?,date=? where itemcode=?");
//
...
st.setFloat(7,sp);
st.setString(8,dt);
st.setString(9,icode);
st.executeUpdate();
JOptionPane.showMessageDialog(null," Item Modify Successfully!");
t1.setText("qw");
t1.setText("");
t2.setText("ew");
t2.setText("");
...
try
{
st=con.prepareStatement("delete from iteminfo where itemcode=?");
st.setString(1,icode);
st.executeUpdate();
JOptionPane.showMessageDialog(null,"Item Details Delete Successfully!");
t1.setText("qw");
t1.setText("");
t2.setText("ew");
t2.setText("");
