...
class Salaryslipgeneration extends Frame implements KeyListener,ActionListener
{
...
JTable t=new JTable();
String head[]={"S_no","EmpId","EmpName","Designation","Salary","No._Of_Days_Work","Net_Salary"};
...
jl.setBounds(300,50,350,50);add(jl);
...
ns=Double.parseDouble(model.getValueAt(i,6).toString());
...
mn=ch.getSelectedItem();
yr=t2.getText();
tday=Integer.parseInt(t3.getText());
try
{
...
{
ed=rs.getString(1);
st1=con.prepareStatement("select designation,salary from empinfo where empid=?");
st1.setString(1,ed);
rs1=st1.executeQuery();
rs1.next();
desg=rs1.getString(1);
sal=rs1.getInt(2);
en=rs.getString(2);
count=rs.getInt(3);
nsal=(sal*count)/tday;
model.addRow(new Object[]{sn,ed,en,desg,sal,count,nsal});
sn++;
}
}
catch(Exception eg)
